...
`Return Value - Returns the number of branches used by the generated codex.`
...
Warning: The branches array must have enough capacity. If the array is too small, the implementation may access memory outside the allocated array, potentially causing a segmentation fault or other undefined behavior.
...
`Return Value - Returns the number of tokens generated by the parser.`
...
Warning: The function does not receive the size of the tokens array. The caller must ensure that enough space is available for every token that may be generated. An undersized array may result in out-of-bounds memory access and undefined behavior.
...
Generated tokens are likewise stored in a pre-allocated lexic_token array:
...
The public API is exposed through include/stack_lexer.h, while the implementation is contained in src/stack_lexer.c.
